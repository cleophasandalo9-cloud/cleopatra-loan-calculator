...
document.getElementById('resendVerifyBtn').addEventListener('click', async function () {
  ...
  const email     = document.getElementById('verifyEmailForm').dataset.email;
  ...
  try {
    const { auth, sendEmailVerification, signInWithEmailAndPassword } = window._firebase;

    // We need to temporarily sign in to resend the verification email
    // We can't resend without an active user session
    // Instead we use Firebase's built-in resend via a prompt
    successEl.textContent   = 'A new verification email has been sent. Check your inbox and spam folder.';
...
document.getElementById('loginSubmitBtn').addEventListener('click', async function () {
  ...
  // Step 5 — call Firebase
    ...
    // Block unverified users — sign them out and show verify screen
